...
export const meta: MetaFunction = ({ matches }) => {
  ...
  // Prefer words[3]; fallback to array order (3..9 -> index 0 for three)
  const count =
    words?.[LENGTH]?.length ??
    (words ? Object.values(words)[0]?.length ?? undefined : undefined);
...
export default function ThreeLetterWords() {
  ...
  const list: string[] =
    ...
    (words ? (Object.values(words)[0] as string[]) : []) ??
    [];
