...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    console.error("GetWordsForSkull failed in 9-letter route:", e);
  }
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  const desc = count
    ? `Access ${count.toLocaleString(
        "en-US"
      )} 9-letter words for crosswords, anagrams, and puzzles. Great for mastering tough word challenges and expanding vocab.`
    : "Access 9-letter words for crosswords, anagrams, and puzzles. Great for mastering tough word challenges and expanding vocab.";

  const url =
    data?.canonical ??
    "https://www.wordskull.com/words-list/all-9-letter-words-for-word-games";
...
export default function NineLetterWords() {
  ...
  const handleCopy = useCallback(async (word: string) => {
    try {
      ...
      // No-op; you could set an error state if you want
    }
  }, []);
  ...
  return (
    <div className="flex flex-col justify-center items-center mt-10">
      ...
      {/* SR-only live region for screenreader feedback */}
      ...
      <main className="mx-auto flex max-w-[1200px] flex-col w-full px-4">
