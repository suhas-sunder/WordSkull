...
/* ===== META (unique, keyword-rich, uses count if available) ===== */
export const meta: MetaFunction = ({ matches }) => {
  const root = matches.find((m) => m.id === "root") as Match | undefined;
  const words = root?.data?.words;
  const count =
    (words?.[LENGTH] && words[LENGTH].length) ??
    (words ? Object.values(words)[LENGTH - 3]?.length : undefined);
    ...
    <div className="flex flex-col justify-center items-center mt-10">
      ...
      <main className="flex max-w-[1200px] flex-col w-full">
        ...
        >
          ...
          {/* Semantic list; keep your grid styling */}
          ...
          {/* Minimal internal nav (UX + crawlability) */}
